 ...
 * Updated: Support role admin / user
 */
...
$q_admin = mysqli_query($koneksi,
    "SELECT * FROM admin WHERE username = '" .
    mysqli_real_escape_string($koneksi, $username_session) . "' LIMIT 1"
);
...
$role_label     = is_admin() ? 'Administrator' : 'User (Read-Only)';
$role_color     = is_admin() ? '#22C55E' : '#F97316';

// ── Sinkronkan session role dari DB (aman) ─────────────────────
if (isset($current_admin['role'])) {
    $_SESSION['role'] = $current_admin['role'];
}
...
if (isset($_GET['baca_notif'])) {
    ...
    if (isset($_SESSION['notifikasi'])) {
        foreach ($_SESSION['notifikasi'] as &$n) {
            if ($n['id'] === $nid || $nid === 'semua') $n['dibaca'] = true;
        }
    }
...
<head>
  ...
  <style>
    ...
    .xtm-avatar {
      ...
      letter-spacing: 0.5px; flex-shrink: 0; user-select: none; line-height: 1;
    }
    ...
    .xtm-dropdown {
      ...
      box-shadow: 0 8px 30px rgba(0,0,0,.12); overflow: hidden; min-width: 340px;
    }
    ...
    .xtm-profile-header { padding: 16px; border-bottom: 1px solid #F1F5F9; display: flex; align-items: center; gap: 10px; }
    .xtm-badge {
      position: absolute; top: 2px; right: 2px; min-width: 16px; height: 16px;
      background: #EF4444; color: #fff; border-radius: 8px; font-size: 9px; font-weight: 700;
    ...
    #xtm-search-overlay {
      ...
      z-index: 9999; display: none; align-items: flex-start; justify-content: center; padding-top: 80px;
    }
    #xtm-search-overlay.show { display: flex; }
    #xtm-search-box { background: #fff; border-radius: 16px; width: 100%; max-width: 580px; box-shadow: 0 20px 60px rgba(0,0,0,.18); overflow: hidden; }
    #xtm-search-input { border: none; outline: none; font-family: 'Outfit', sans-serif; font-size: 16px; flex: 1; color: #0F172A; }
    .xtm-search-result {
      padding: 11px 20px; display: flex; gap: 12px; align-items: center; cursor: pointer;
      border-bottom: 1px solid #F8FAFC; text-decoration: none; color: inherit; transition: background .1s;
    }
    ...
    .role-badge.admin { background: #DCFCE7; color: #16A34A; }
    .role-badge.user  { background: #FFF7ED; color: #C2410C; }
    ...
    <div class="container-fluid">
      ...
      <ul class="navbar-nav ms-auto align-items-center gap-1">
        ...
        <li class="nav-item dropdown">
          <a class="nav-link position-relative" data-bs-toggle="dropdown" href="#" title="Notifikasi">
            <i class="bi bi-bell-fill fs-6"></i>
            <?php if ($unread_notif > 0): ?>
              <span class="xtm-badge"><?= min($unread_notif, 99) ?></span>
            <?php endif; ?>
          </a>
          <div class="dropdown-menu dropdown-menu-end xtm-dropdown p-0">
            <div class="dropdown-header-title">
              <span>Notifikasi</span>
              <?php if ($unread_notif > 0): ?>
                <a href="?baca_notif=semua" class="text-primary text-decoration-none" style="font-size:12px;font-weight:600;">Tandai semua dibaca</a>
              <?php endif; ?>
            </div>
            <div style="max-height:360px;overflow-y:auto;">
              <?php if (empty($notifikasi_list)): ?>
                <div class="p-4 text-center text-muted" style="font-size:13px;">
                  <i class="bi bi-bell-slash fs-4 d-block mb-2"></i>Belum ada notifikasi
                </div>
              <?php else: ?>
                ...
                <a href="?baca_notif=<?= htmlspecialchars($notif['id']) ?>"
                  ...
                  <div class="xtm-notif-icon"><i class="bi <?= $icon_class ?>"></i></div>
                  <div style="flex:1;">
                    <div style="font-weight:<?= !$notif['dibaca']?'700':'500' ?>;font-size:13px;color:#0F172A;"><?= htmlspecialchars($notif['judul']) ?></div>
                    <div style="font-size:12px;color:#64748B;margin-top:2px;"><?= htmlspecialchars($notif['body']) ?></div>
                    <div style="font-size:11px;color:#94A3B8;margin-top:4px;"><?= format_waktu_relatif($notif['waktu']) ?></div>
                  </div>
            ...
            <div style="padding:10px 16px;border-top:1px solid #F1F5F9;text-align:center;">
              <a href="?baca_notif=semua" class="text-decoration-none" style="font-size:12px;color:#64748B;">Lihat semua notifikasi</a>
            </div>
          </div>
        </li>
          ...
          <a href="#" class="nav-link dropdown-toggle d-flex align-items-center gap-2 ps-1 pe-2"
            ...
            <div class="d-none d-md-block" style="line-height:1.2;">
              ...
              <div style="font-size:11px;color:<?= $role_color ?>;font-weight:600;">
                <?= is_admin() ? 'Administrator' : 'User (Read-Only)' ?>
              </div>
            </div>
              ...
              <div>
                ...
                <div class="mt-1">
                  <span class="role-badge <?= is_admin() ? 'admin' : 'user' ?>">
                    <i class="bi <?= is_admin() ? 'bi-shield-fill-check' : 'bi-eye-fill' ?>"></i>
                    <?= is_admin() ? 'Admin' : 'User' ?>
                  </span>
                </div>
              </div>
  ...
  <div id="xtm-search-overlay">
    <div id="xtm-search-box">
      <div class="d-flex align-items-center px-3 py-2 border-bottom" style="gap:10px;">
        ...
        <button id="btn-close-search" class="btn btn-sm btn-light" style="border-radius:8px;"><i class="bi bi-x-lg"></i></button>
      </div>
      <div id="xtm-search-results" style="max-height:400px;overflow-y:auto;">
        <div class="p-4 text-center text-muted" style="font-size:13px;">
          <i class="bi bi-search d-block fs-4 mb-2 opacity-25"></i>Ketik minimal 2 karakter…
        </div>
      </div>
      <div class="px-4 py-2 border-top d-flex gap-3" style="font-size:11px;color:#94A3B8;">
        <span><kbd>↵</kbd> Buka</span><span><kbd>ESC</kbd> Tutup</span>
      </div>
    </div>
  </div>